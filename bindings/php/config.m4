...
if test "$PHP_INAPI" != "no"; then
    ...
    PHP_CHECK_LIBRARY(inapi, host_new,
    [
        PHP_ADD_INCLUDE($INAPI_INC_DIR)
        PHP_ADD_LIBRARY_WITH_PATH(inapi, $INAPI_LIB_DIR, INAPI_SHARED_LIBADD)
        AC_DEFINE(HAVE_INAPI, 1, [whether host_new function exists])
    ],[
        AC_MSG_ERROR([host_new function not found in libinapi])
    ],[
        -L$INAPI_LIB_DIR -R$INAPI_LIB_DIR
    ])

    PHP_SUBST(INAPI_SHARED_LIBADD)
    PHP_NEW_EXTENSION(inapi, php_inapi.c wrappers/command.c wrappers/data.c wrappers/directory.c wrappers/host.c wrappers/package.c wrappers/service.c wrappers/telemetry.c wrappers/file.c wrappers/template.c, $ext_shared)
fi
